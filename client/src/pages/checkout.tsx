    ...
    return (
      <div className="max-w-xl mx-auto px-4 sm:px-6 lg:px-8 py-20 flex flex-col items-center text-center" data-testid="page-order-success">
        ...
        <p className="text-muted-foreground mt-2 text-sm max-w-sm">
          Thank you, {formData.fullName}! Your order has been placed successfully. A confirmation email has been sent to {formData.email}.
        </p>
        <Card className="mt-6 p-4 w-full text-left">
        ...
        <div className="flex flex-wrap gap-3 mt-8">
          <Link href="/shop">
            <Button className="gap-2" data-testid="button-continue-shopping-success">
            ...
            <div className="space-y-3">
              {items.map((item) => (
                <div key={item.product.id} className="flex items-center gap-3">
                  ...
                  <p className="text-sm font-medium shrink-0">{"\u20B9"} {item.product.price * item.quantity}</p>
                </div>
              ))}
            </div>
